...
public class Filter {
    ...
    public static void main(String[] args) {
        ...
        alunos.stream().filter(aprovado).
                map(saudacao).forEach(System.out::println);

        //alunos.stream().filter(aluno -> aluno.nota >= 7).
          //      map(aluno -> "Parabéns " + aluno.nome + "! Você foi aprovado(a)!").forEach(System.out::println);
